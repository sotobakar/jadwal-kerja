...
<h1 class="text-center">Create Employee Shift</h1>
@stop
  ...
  <div class="row">
    <div class="col-lg-6">
      ...
      <x-adminlte-card title="Employee Shift Form" theme="light" icon="fas fa-lg fa-bell">
        <form action={{ route('employees.shifts') }} method="POST">
          @csrf
          ...
          <x-adminlte-select2 name="shift" label="Shift" label-class="text-black">
            ...
            <option value={{ $shift->id }}>{{ $shift->code . ' - ' . $shift->start_time . ' - ' . $shift->end_time }}
            </option>
            @endforeach
          </x-adminlte-select2>
          @php
          ...
          <x-adminlte-button class="d-flex ml-auto" theme="primary" label="Submit" type="submit" />
        </form>
      </x-adminlte-card>
    </div>
    <div class="col-lg-6">
